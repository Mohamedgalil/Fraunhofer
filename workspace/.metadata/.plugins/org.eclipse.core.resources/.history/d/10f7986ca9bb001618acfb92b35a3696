...
import liaa.dct.bor.control.Utilities;
...
public class PageOne extends WizardPage {
	...
	public void handleEvent(SelectionEvent e) {
		if (e.widget == btnImport) {
			createFileDialog(SWT.MULTI);
		}
		if (e.widget == btnBoR) {
			createFileDialog(SWT.SINGLE);
		}
		if (e.widget == btnBoP) {
			createFileDialog(SWT.SINGLE);
	...
	public String[] createFileDialog(int type) {
		FileDialog fileDialog = new FileDialog(new Shell(), type);
		...
		for (int i = 0; i < selectedFiles.length; i++) {
			selectedFiles[i] = filterPath + "/" + selectedFiles[i];
			Utilities.validateBoR(selectedFiles[i]);
